...
async function createAndSignTransaction() {
  ...
  // 提取签名数据并保存
  const signatures = [...signedSafeTransaction.signatures.entries()].map(([signer, signature]) => ({
    signer,
    data: signature.data,
  }));

  const serializedSignedTransaction = {
    to: signedSafeTransaction.data.to,
    value: signedSafeTransaction.data.value,
    data: signedSafeTransaction.data.data,
    operation: signedSafeTransaction.data.operation,
    safeTxGas: signedSafeTransaction.data.safeTxGas,
    baseGas: signedSafeTransaction.data.baseGas,
    gasPrice: signedSafeTransaction.data.gasPrice,
    gasToken: signedSafeTransaction.data.gasToken,
    refundReceiver: signedSafeTransaction.data.refundReceiver,
