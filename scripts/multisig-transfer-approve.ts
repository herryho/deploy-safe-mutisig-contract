...
function arrayify(hexString: string) {
...
async function approveTransaction() {
  try {
    ...
    // 确保 `signatures` 是一个 Map 对象
    const signatures = new Map();
    for (const sig of signedSafeTransaction.signatures) {
      signatures.set(sig.signer, sig);
    }

    // 检查并去重
    if (!signatures.has(secondSignature.signer)) {
      signatures.set(secondSignature.signer, secondSignature);
    } else {
      console.log(`Signature from ${secondSignature.signer} already exists.`);
    }

    console.log('All Signatures:', JSON.stringify([...signatures.values()], null, 2)); // 调试打印所有签名

    console.log('Successfully approved the transaction.');

    fs.writeFileSync('approvedSafeTransaction.json', JSON.stringify({ signedSafeTransaction: { ...signedSafeTransaction, signatures: [...signatures.values()] }, safeTxHash }, null, 2));
  } catch (err) {
